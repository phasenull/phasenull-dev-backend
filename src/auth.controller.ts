...
const OAuthController = new Hono<CustomContext>()

OAuthController.post("/callback", async (c) => {
	...
	if (
		!(
			...
			response.scope.includes("users.email")
		)
	)
	...
	const [result] = await db
		...
		.values({
			...
			account_username: user?.data.username,
			data: user.data || {}
		})
		.returning()
...
OAuthController.get("/authorize", async (c) => {
	if (!c.env.JWT_SECRET)
		return c.json(
	...
	const [entry] = await db
		...
		.returning()
	const is_dev = !!c.req.query("dev")
	const jwt = await sign(
		{
			iat: Math.floor(Date.now() / 1000),
			exp: Math.floor(Date.now() / 1000) + 15 * 60,
			secret: entry.id,
			is_dev: !!c.req.query("dev")
		},
		c.env.JWT_SECRET
	)
	...
	const uri = is_dev ? "http://localhost:5173" : c.env.PORTFOLIO_URL
	oauth_url.searchParams.set("redirect_uri", `${uri}/admin/oauth/callback`)
	oauth_url.searchParams.set("scope", "tweet.read users.read users.email")
	...
	console.log("returning url", c.req.header("Host"), oauth_url.toString())
	return c.json({
		success: true,
